...
#let _cvHeader(
  ...
  let headerInfoStyle(str) = {
    text(size: 10pt, fill: black, str)
  }
  let headerQuoteStyle(str) = {
    smallcaps(text(size: 10pt, weight: "medium", fill: accentColor, str))
  }
  ...
  let makeHeaderInfo() = {
    ...
    let personalInfoIconsBis = (
      phone: "icons/phone.svg",
      github: "icons/github.svg",
      email: "icons/mail.svg",
      homepage: "icons/website.svg"
    )
    let n = 1
    for (k, v) in personalInfo {
      ...
      if k.contains("custom") {
        ...
        box({
          // Adds icons
          box(image(personalInfoIconsBis.at(k), height: 1em), baseline: 2pt)
          h(5pt)
          // Adds hyperlinks
          if k == "email" {
            link("mailto:" + v)[#v]
  ...
  let makeHeaderNameSection() = table(
    ...
    [#v(-5pt)#headerQuoteStyle(headerQuote)],
    [#v(-5pt)#headerInfoStyle(makeHeaderInfo())#v(8pt)],
  )
...
#let cvSection(
  ...
  let sectionTitleStyle(str, color: black) = {
    smallcaps(text(size: 15pt, weight: "bold", fill: color, str))
  }
  ...
  h(8pt)
  box(width: 1fr, line(stroke: (thickness: 1pt, dash: (0pt, 6pt), cap: "round"), length: 100%))
  // box(width: 1fr, line(stroke: (thickness: 1pt, dash: (0pt, 6pt), cap: "round"), length: 100%, start: (+8pt, -.5em)))
}
...
#let cvEntry(
  ...
  let entryA2Style(str) = {
    align(
      right,
      text(weight: "light", style: "oblique", str),
    )
  }
  let entryB1Style(str) = {
    text(size: 9pt, fill: accentColor, weight: 400, smallcaps(str))
  }
  let entryB2Style(str) = {
    align(
      right,
      text(size: 9pt, weight: "medium", fill: gray, style: "oblique", str),
    )
  }
  ...
  let entryTagStyle(str) = {
    align(center, text(size: 9pt, weight: "regular", str))
  }
...
#let cvSkill(type: "Type", info: "Info") = {
  let skillTypeStyle(str) = {
    align(right, text(size: 11pt, weight: "bold", str))
  }
